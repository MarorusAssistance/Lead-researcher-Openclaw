...
- CTO email body:
  - `Hi {firstName}. I saw that {company} works across AI and software delivery, so this may be relevant. I help small IT teams build agentic workflows that remove repetitive internal work across delivery, research, and operations. If useful, I can send one concrete use case that looks plausible for a team like yours.`

- Valid length patterns:
  - `connectionNoteDraft` must always be 200 chars or fewer
  - `emailBodyDraft` must always be 70-130 words
  - if unsure, shorten the connect note and expand the email body with one grounded sentence
